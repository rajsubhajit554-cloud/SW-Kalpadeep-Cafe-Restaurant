...
const modalOrderHotlineSection = document.getElementById('modal-order-hotline');
...
// Two-Way Scroll for Modal Navigation Button (Down to Bottom / Up to Top)
...
function isNearModalBottom() {
    if (!modalBody) return false;
    const isScrolledToBottom = (modalBody.scrollTop + modalBody.clientHeight >= modalBody.scrollHeight - 150);
    if (modalOrderHotlineSection) {
        const modalBodyRect = modalBody.getBoundingClientRect();
        const hotlineRect = modalOrderHotlineSection.getBoundingClientRect();
        return (hotlineRect.top <= modalBodyRect.bottom - 120) || isScrolledToBottom;
    }
    return isScrolledToBottom;
}

if (modalTimingBtn && modalBody) {
    modalTimingBtn.addEventListener('click', (e) => {
        e.preventDefault();
        if (isNearModalBottom()) {
            modalBody.scrollTo({ top: 0, behavior: 'smooth' });
        } else {
            if (modalOrderHotlineSection) {
                modalOrderHotlineSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
            } else {
